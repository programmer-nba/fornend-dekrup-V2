  ...
  <div class="card">
    <div class="flex flex-wrap justify-content-center card-container">
      <DataTable :value="categorys" :scrollable="true" scrollHeight="flex">
        ...
        <Column field="name" header="หมวดหมู่" class="column-responsive"></Column>
        <Column :exportable="false" style="min-width: 10rem">
          <template #body="item">
            <CategoryProduct :cat_id="item.data._id" :category="item.data" :categorys="categorys" />
          </template>
        </Column>
      </DataTable>
    </div>
...
export default {
  ...
  created() {
    document.title = "Category Product | Dekrub Shop";
  },
  setup() {
    const categorys = ref([]);
